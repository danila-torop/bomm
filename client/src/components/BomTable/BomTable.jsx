...
function BomTable() {
  ...
  const fetchData = () => {
    fetch("http://localhost:8080/api/assy")
  ...
  const handleDelete = () => {
    selectedRows.forEach((id) => {
      fetch(`http://localhost:8080/api/assy/${id}`, {
        method: "DELETE",
      })
        .then((response) => {
          if (response.ok) {
  ...
  const handleEdit = (id) => {
    ...
    if (itemToEdit) {
      ...
      setEditedRecord({
        ...
        parent_id: itemToEdit.parent_id,
      });
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    fetch("http://localhost:8080/api/assy", {
      ...
      .then((result) => {
        ...
        setIsAdding(false);
        setNewRecord({ designation: "", name: "", parent_id: "", });
        fetchData();
      })
